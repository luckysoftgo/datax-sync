...
public class JdbcDatasourceQueryController extends ApiController {
    ...
    /**
     * 根据数据源id获取得到kylin数据的sql
     *
     * @param datasourceId
     * @return
     */
    @GetMapping("/getKylinSql")
    @ApiOperation("根据数据源id获取得到kylin数据的sql")
    public R<String> getKylinSql(Long datasourceId,String tableName) {
        return success(jdbcDatasourceQueryService.getKylinSql(datasourceId,tableName));
    }
     ...
     * @return
     */
    @GetMapping("/getElasticColumns")
    @ApiOperation("根据数据源id和表名获得es的操作列信息")
    public R<List<String>> getElasticColumns(Long datasourceId, String tableName) {
        return success(jdbcDatasourceQueryService.getElasticColumns(datasourceId, tableName));
    }
